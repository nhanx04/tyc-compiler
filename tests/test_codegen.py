...
def test_101_struct_member_update_in_function_then_return_sum():
    ast = Program([
        StructDecl("Point", [MemberDecl(IntType(), "x"), MemberDecl(IntType(), "y")]),
        FuncDecl(IntType(), "adjust", [Param(StructType("Point"), "p")], BlockStmt([
            ExprStmt(AssignExpr(MemberAccess(Identifier("p"), "x"), BinaryOp(MemberAccess(Identifier("p"), "x"), "+", IntLiteral(1)))),
            ExprStmt(AssignExpr(MemberAccess(Identifier("p"), "y"), BinaryOp(MemberAccess(Identifier("p"), "y"), "+", IntLiteral(1)))),
            ReturnStmt(BinaryOp(MemberAccess(Identifier("p"), "x"), "+", MemberAccess(Identifier("p"), "y")))
        ])),
        FuncDecl(VoidType(), "main", [], BlockStmt([
            VarDecl(StructType("Point"), "p", StructLiteral([IntLiteral(20), IntLiteral(20)])),
            ExprStmt(FuncCall("printInt", [FuncCall("adjust", [Identifier("p")])]))
        ]))
    ])
    assert CodeGenerator().generate_and_run(ast) == "42"


def test_102_nested_for_and_while_control_flow():
    ast = Program([
        FuncDecl(VoidType(), "main", [], BlockStmt([
            VarDecl(IntType(), "i", IntLiteral(0)),
            ForStmt(VarDecl(IntType(), "j", IntLiteral(0)), BinaryOp(Identifier("j"), "<", IntLiteral(3)), AssignExpr(Identifier("j"), BinaryOp(Identifier("j"), "+", IntLiteral(1))), BlockStmt([
                WhileStmt(BinaryOp(Identifier("i"), "<", IntLiteral(1)), BlockStmt([
                    ExprStmt(FuncCall("printString", [StringLiteral("a")])) ,
    ...
    assert CodeGenerator().generate_and_run(ast) == "abb"


def test_103_switch_after_loop():
    ast = Program([
        FuncDecl(VoidType(), "main", [], BlockStmt([
            VarDecl(IntType(), "x", IntLiteral(0)),
            WhileStmt(BinaryOp(Identifier("x"), "<", IntLiteral(2)), BlockStmt([
                ExprStmt(AssignExpr(Identifier("x"), BinaryOp(Identifier("x"), "+", IntLiteral(1))))
            ])),
            SwitchStmt(Identifier("x"), [
                CaseStmt(IntLiteral(2), [ExprStmt(FuncCall("printString", [StringLiteral("ok")]))])
            ], DefaultStmt([ExprStmt(FuncCall("printString", [StringLiteral("bad")]))]))
        ]))
    ])
    assert CodeGenerator().generate_and_run(ast) == "ok"


def test_104_string_function_chain():
    ast = Program([
        FuncDecl(StringType(), "a", [], BlockStmt([ReturnStmt(StringLiteral("x"))])),
        FuncDecl(StringType(), "b", [Param(StringType(), "s")], BlockStmt([ReturnStmt(BinaryOp(Identifier("s"), "+", StringLiteral("y")))])),
        FuncDecl(VoidType(), "main", [], BlockStmt([
            ExprStmt(FuncCall("printString", [FuncCall("b", [FuncCall("a", [])])]))
        ]))
    ])
    assert CodeGenerator().generate_and_run(ast) == "xy"


def test_105_postfix_decrement_on_member_access():
    ast = Program([
        StructDecl("Counter", [MemberDecl(IntType(), "n")]),
        FuncDecl(VoidType(), "main", [], BlockStmt([
            VarDecl(StructType("Counter"), "c", StructLiteral([IntLiteral(5)])),
            ExprStmt(FuncCall("printInt", [PostfixOp("--", MemberAccess(Identifier("c"), "n"))])),
            ExprStmt(FuncCall("printInt", [MemberAccess(Identifier("c"), "n")]))
        ]))
    ])
...
def test_106_float_comparison_if_else():
    ast = Program([
        FuncDecl(VoidType(), "main", [], BlockStmt([
            IfStmt(BinaryOp(FloatLiteral(1.0), ">", FloatLiteral(0.5)),
                   ExprStmt(FuncCall("printString", [StringLiteral("a")])),
                   ExprStmt(FuncCall("printString", [StringLiteral("b")])))
        ]))
    ])
...
def test_107_void_function_with_breakable_loop():
    ast = Program([
        FuncDecl(VoidType(), "printer", [], BlockStmt([
            VarDecl(IntType(), "i", IntLiteral(0)),
            WhileStmt(BinaryOp(Identifier("i"), "<", IntLiteral(2)), BlockStmt([
                ExprStmt(FuncCall("printString", [StringLiteral("p")])) ,
                BreakStmt()
            ]))
        ])),
        FuncDecl(VoidType(), "main", [], BlockStmt([
            ExprStmt(FuncCall("printer", [])),
            ExprStmt(FuncCall("printString", [StringLiteral("q")]))
        ]))
    ])
    assert CodeGenerator().generate_and_run(ast) == "pq"


def test_108_assignment_expression_chain():
    ast = Program([
        FuncDecl(VoidType(), "main", [], BlockStmt([
            VarDecl(IntType(), "x", IntLiteral(0)),
            ExprStmt(AssignExpr(Identifier("x"), AssignExpr(Identifier("x"), IntLiteral(42)))),
            ExprStmt(FuncCall("printInt", [Identifier("x")]))
        ]))
    ])
    assert CodeGenerator().generate_and_run(ast) == "42"
...
def test_109_nested_struct_and_function_mix():
    ast = Program([
        StructDecl("Inner", [MemberDecl(IntType(), "v")]),
        StructDecl("Outer", [MemberDecl(StructType("Inner"), "inner")]),
        FuncDecl(IntType(), "getv", [Param(StructType("Outer"), "o")], BlockStmt([
            ReturnStmt(MemberAccess(MemberAccess(Identifier("o"), "inner"), "v"))
        ])),
        FuncDecl(VoidType(), "main", [], BlockStmt([
            VarDecl(StructType("Outer"), "o", StructLiteral([StructLiteral([IntLiteral(42)])])),
            ExprStmt(FuncCall("printInt", [FuncCall("getv", [Identifier("o")])]))
        ]))
    ])
    assert CodeGenerator().generate_and_run(ast) == "42"


def test_110_final_extra_coverage_case():
    ast = Program([
        FuncDecl(IntType(), "f", [Param(IntType(), "x")], BlockStmt([
            ReturnStmt(BinaryOp(BinaryOp(Identifier("x"), "+", IntLiteral(20)), "+", IntLiteral(20)))
        ])),
        FuncDecl(VoidType(), "main", [], BlockStmt([
            ExprStmt(FuncCall("printInt", [FuncCall("f", [IntLiteral(2)])]))
        ]))
    ])
    assert CodeGenerator().generate_and_run(ast) == "42"
